...
def player_move(arr, score):
    while True:
        key=getkey().decode()
        if key == 'w' and game_over_v < rows:
            tempscore, arr = move_up(matrix)
            score += tempscore
        elif key== 's' and game_over_v < rows:
            tempscore, arr = move_down(matrix)
            score += tempscore
        elif key == 'd' and game_over_h < columns:
            tempscore, arr = move_right(matrix)
            score += tempscore
        elif key == 'a' and game_over_h < columns:
            tempscore, arr = move_left(matrix)
            score += tempscore
        else:
            continue
        return score, arr
...
while game == True and game_over < rows + columns: 
    ...
    score, matrix = player_move(matrix, score)
    computer_move(matrix)
    print_matrix(matrix)
else:
